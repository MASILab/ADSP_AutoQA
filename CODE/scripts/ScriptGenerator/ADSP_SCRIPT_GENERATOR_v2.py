...
class BedpostX_plus_DWI_plus_TractsegGenerator(ScriptGenerator):
    ...
    def bedpostx_plus_dwi_plus_tractseg_script_generate(self, script, session_input, session_output, **kwargs):
        ...
        if self.setup.args.accre_gpu:
            script.write("time singularity run --nv -B {bedpost}:{bedpost} -B {accrehome}:{accrehome} -B {outdir}:{outdir} {simg} TractSeg -i {bedpost}/dyads1.nii.gz -o {outdir}\n".format(bedpost=bedpost_ouputs, outdir=tractsegdir, simg=ts_simg, accrehome=kwargs['accre_home']))
            script.write("time singularity run --nv -B {bedpost}:{bedpost} -B {accrehome}:{accrehome} -B {outdir}:{outdir} {simg} TractSeg -i {bedpost}/dyads1.nii.gz -o {outdir} --output_type endings_segmentation\n".format(bedpost=bedpost_ouputs, outdir=tractsegdir, simg=ts_simg, accrehome=kwargs['accre_home']))
            script.write("time singularity run --nv -B {bedpost}:{bedpost} -B {accrehome}:{accrehome} -B {outdir}:{outdir} {simg} TractSeg -i {bedpost}/dyads1.nii.gz -o {outdir} --output_type TOM\n".format(bedpost=bedpost_ouputs, outdir=tractsegdir, simg=ts_simg, accrehome=kwargs['accre_home']))
            script.write("time singularity run --nv -B {bedpost}:{bedpost} -B {accrehome}:{accrehome} -B {outdir}:{outdir} {simg} Tracking -i {bedpost}/dyads1.nii.gz -o {outdir} --tracking_format tck\n".format(bedpost=bedpost_ouputs, outdir=tractsegdir, simg=ts_simg, accrehome=kwargs['accre_home']))

        else:
            script.write("time singularity run -B {bedpost}:{bedpost} -B {accrehome}:{accrehome} -B {outdir}:{outdir} {simg} TractSeg -i {bedpost}/dyads1.nii.gz -o {outdir}\n".format(bedpost=bedpost_ouputs, outdir=tractsegdir, simg=ts_simg, accrehome=kwargs['accre_home']))
            script.write("time singularity run -B {bedpost}:{bedpost} -B {accrehome}:{accrehome} -B {outdir}:{outdir} {simg} TractSeg -i {bedpost}/dyads1.nii.gz -o {outdir} --output_type endings_segmentation\n".format(bedpost=bedpost_ouputs, outdir=tractsegdir, simg=ts_simg, accrehome=kwargs['accre_home']))
            script.write("time singularity run -B {bedpost}:{bedpost} -B {accrehome}:{accrehome} -B {outdir}:{outdir} {simg} TractSeg -i {bedpost}/dyads1.nii.gz -o {outdir} --output_type TOM\n".format(bedpost=bedpost_ouputs, outdir=tractsegdir, simg=ts_simg, accrehome=kwargs['accre_home']))
            script.write("time singularity run -B {bedpost}:{bedpost} -B {accrehome}:{accrehome} -B {outdir}:{outdir} {simg} Tracking -i {bedpost}/dyads1.nii.gz -o {outdir} --tracking_format tck\n".format(bedpost=bedpost_ouputs, outdir=tractsegdir, simg=ts_simg, accrehome=kwargs['accre_home']))
